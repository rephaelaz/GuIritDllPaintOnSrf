...
{
	{
		0,
			...
			IrtMdlrBeepExample,
			NULL,
			IRT_MDLR_PARAM_POINT_SIZE_CONTROL | IRT_MDLR_PARAM_VIEW_CHANGES_UDPATE | IRT_MDLR_PARAM_INTERMEDIATE_UPDATE_DFLT_ON, IRT_MDLR_NUMERIC_EXPR,
			3, IRT_MDLR_PARAM_EXACT,
		{
...
static void IrtMdlrBeepExample(IrtMdlrFuncInfoClass *FI)
{
	...
	if (FI -> InvocationNumber == 0 && GlblExampleFI == NULL) {
		GuIritMdlrDllPushMouseEventFunc(
			...
			FI);
		GuIritMdlrDllPushGenericEventFunc(
			FI,
